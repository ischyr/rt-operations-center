import { Box, Flex, Text, Heading, Badge, Progress } from '@chakra-ui/react';
  ...
  return (
    ...
    >
      ...
      <Flex justify="space-between" align="flex-start" mb={1}>
        <Box>
          <Heading fontSize="lg" fontWeight="bold" color="white">{name}</Heading>
          <Text fontSize="12px" color="gray.500" mt="2px">{client}</Text>
        </Box>
        <Box
      ...
      <Flex gap={5} mt={2} mb={4} flexWrap="wrap">
        <Flex align="center" gap={1}>
          <CalendarIcon boxSize={3} color="gray.600" />
          <Text fontSize="11px" color="gray.500">Started {started}</Text>
        </Flex>
        <Flex align="center" gap={1}>
          <CalendarIcon boxSize={3} color="gray.600" />
          <Text fontSize="11px" color="gray.500">Ends {ends}</Text>
        </Flex>
        <Flex align="center" gap={1}>
          <AtSignIcon boxSize={3} color="gray.600" />
          <Text fontSize="11px" color="gray.500">{operators}</Text>
        </Flex>
        <Flex align="center" gap={1}>
          <InfoIcon boxSize={3} color="gray.600" />
          <Text fontSize="11px" color="gray.500">{phase}</Text>
        </Flex>
      </Flex>

      {/* Progress */}
      <Flex justify="space-between" align="center" mb={1}>
        <Text fontSize="10px" color="gray.600" letterSpacing="wider" textTransform="uppercase">Overall Progress</Text>
        <Text fontSize="12px" fontWeight="bold" color="gray.300">{progress}%</Text>
      </Flex>
      <Box
        h="6px" borderRadius="full" bg="rgba(255,255,255,0.06)" mb={4} overflow="hidden"
      >
        <Box
